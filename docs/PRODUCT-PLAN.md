...
4. **内容生产的单人风险**：周报/信件若依赖人写，必然断更。原则：**机器生成全文，人只做 review + 外发**。Stage 17/18 已是这个形态，要保持。
...
新增 `stages/20-pages.mjs`（或在 04 基础上演进），零依赖 Markdown→HTML（信件/周报是 md，写一个 100 行的渲染器即可，不引依赖）：
...
M2（10月）      Stage 19 + 双栏周报 + 雷达 v0/v1（视声明率定路线）+ /dynamics 页
...
| 新增风险 | 等级 | 缓解 |
|---|---|---|
| 内容断更（单人运维） | 高 | 机器生成全文 + 人只 review；周报模板化已完成（Stage 18），断更即产品死亡 |
| rc 雷达声明率不足（实测：当前 0 插件有版本契约字段） | 高 | 先普查再定路线；v1 代码面方案兜底 |
| 站点多页化后 CI 构建变慢/产物膨胀 | 中 | 信件 diff 驱动生成；产物大小入 CI 日志监控 |
| 官方动态数据源变动（repo 改名/私有） | 低 | Stage 19 失败降级为"本周无数据"，不阻塞管线 |
