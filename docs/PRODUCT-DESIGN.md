...
dsh-insights.com
│
├── /                汇总门户（门面）—— 搜索直达 + KPI + 最新入库 + 本周速览 + 官方动态 + 场景速配 + 全站导览
├── /dashboard/      仪表盘 —— 生态趋势 / 质量分布 / 榜单 + 插件库（#browse：全量搜索 / 筛选 / 排序）
├── /plugins/        （已并入 /dashboard/，旧 URL 留 0 秒跳转）
├── /p/<o>/<r>/      插件页（触达）—— 每插件一页：健康分 + 六维画像 + 扣分明细 + npm + 同类 + 徽章
├── /weekly/         周报（引擎）—— 双栏阅读器（MD/PDF/PNG 导出）+ 存档 + RSS
├── /dynamics/       官方动态（v0 已上线）—— dsh 官方演进时间线 + rc 兼容雷达（M2）
├── /scenarios/      场景推荐 —— 按使用场景选插件（质量首选 + 新入场）
├── /authors/        作者榜 —— 多产 / 飙升榜单 + 协作关系图
...
四个交付物各司其职：**仪表盘是门面，周报是增长引擎，插件页与「致作者的信」（外发物料）是作者触达，/data 与徽章是被吸收的接口。**

## 二、页面与模块设计

### 2.1 `/dashboard/` 仪表盘（含插件库，2026-09-06 合并）
...
| 04 插件库（#browse，原 /plugins/ 已并入） | 全量表：搜索/筛选/排序；点击行进 `/p/` 详情页（抽屉已移除） | 内联 rows | 每日 CI |

已有基础：2026-09-06 完成 UI 重构（极简精确方向），同日 /plugins/ 并入仪表盘（旧 URL 留 0 秒跳转 + canonical）。后续增量：i18n 维度列、雷达预警标记（M2）。

### 2.2 `/p/<owner>/<repo>/` 插件页（对全量权威集生成；实际实现超出下表——另含作者头像/等级 KPI/六维子分/npm 版本与周下载/深检/同类插件/徽章接入）

每插件一页，数据源 `plugins.jsonl ⨝ enrich.json`（无 enrich 行的新入库插件降级为「评分待生成」）。**「致作者的信」为外发邮件/PR 物料（`data/reports/`，letters.mjs 生成），不再上页。**

| 模块 | 内容 |
|---|---|
| 头部 | 插件名 + 等级大字 + 分数条 + 徽章代码（一键复制 `![health](…)`） |
| 分数证据 | 打分明细（每项 +N 与依据）、缺失数据明说 |
| 收录 / 发布 | awesome/imsai 收录、npm 版本与滞后、周下载 |
| 同类对比 | 同分类 Top5 分位表 |
| LLM 解读 / 深检 | 能力标签、README 宣称、写面/消毒（增量信号） |
...
│   ├── E1 徽章部署仓库数（M3 目标 ≥5）
│   ├── E2 仓库 ★ / 周报转载次数
│   └── E3 站点访问（Umami 统计已启用 2026-09-06；GitHub traffic API 为辅）
└── F. 采纳（北极星的计数）
...
3. ✓ 插件页全量生成（content/letters 输出 → /p/ 路由，4,681 页）
4. ✓ 周报第 1 期 + feed.xml（W33–W36 存档）
5. 02-validate 契约字段普查（决定雷达路线）
6. ✓ metrics.jsonl 自测量接入 CI（周五 append）
