...
- 2026-09-05：**LLM 语义分析上线（D3 首跑）**——15-llm-tags stage，deepseek-v4-flash（用户 key，.env gitignored）。经验：10 行/批 >90s 超时 → 调 4 行/批 + 300s 超时 + 3 分片并行（LLM_SHARDS）；个别行模型漏输出（batch 4 got 3）待补。输出 data/llm.jsonl，不进 health 分数。
